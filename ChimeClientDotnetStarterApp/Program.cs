    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                logger.Information($"EXE Path: {exePath}");
                ...
                logger.Information($"Process created with PID: {pi.dwProcessId}");
                Process process = Process.GetProcessById(pi.dwProcessId);
                if (process != null)
                {
                    logger.Information($"Waiting for the Process {process.ProcessName} to exit");
                    process.WaitForExit();
                }
                if (process == null)
                {
                    logger.Information("Demo Process is closed");
                }
            }
            catch (Exception ex)
            {
            ...
            Console.ReadKey();
            logger.Information("Process is exiting with Exit Code 0");
        }
    }
